...
class Connector:
    ...
    async def update_data(self):
        ...
        async with self.lock:
            ...
            self.measurements.to_csv('data.csv', index=True, header=not file_exists, mode='a' if file_exists else 'w')
...
def _plot_temp_time(df, ax):
    df = df.tail(10)
    ax.clear()
    sns.lineplot(data=df, x='time', y='temperature', marker='o', ax=ax)
    ax.set_xlabel('')
...
def _plot_co2_temp(df, ax):
    df = df.tail(10)
    ax.clear()
    sns.lineplot(data=df, x='time', y='co2', marker='o', color='orange', ax=ax)
    ax.set_xlabel('')
    ax.set_ylabel('CO2')
...
def _plot_temp_nh4(df, ax):
    df = df.tail(10)
    ax.clear()
    sns.lineplot(data=df, x='time', y='temperature', marker='o', ax=ax, label='Temperature')
    sns.lineplot(data=df, x='time', y='nh4', marker='o', ax=ax, label='NH4', color='green')
...
def _plot_acet_alc_co_nh4_toulen(df, ax):
    ...
    sns.lineplot(data=df, x='time', y='alcohol', marker='o', ax=ax, label='Alcohol', color='blue')
    sns.lineplot(data=df, x='time', y='co', marker='o', ax=ax, label='CO', color='red')
    sns.lineplot(data=df, x='time', y='nh4', marker='o', ax=ax, label='NH4', color='green')
    sns.lineplot(data=df, x='time', y='toulen', marker='o', ax=ax, label='Toulen', color='purple')
    ax.set_xlabel('')
    ax.set_ylabel('Values')
...
def switch_plot(plot_type):
    ...
    acet_alc_co_nh4_toulen_button.pack(side='left', padx=10)
...
if __name__ == '__main__':
    ...
    filename = 'data.csv'
    columns_order = ['time', 'aceton', 'alcohol', 'altitude', 'co', 'co2', 'humidity', 'nh4', 'pressure', 'seaLevelPressure', 'temperature', 'toulen']
    ...
    co2_temp_button = tk.Button(
        ...
        command=lambda: switch_plot('co2')
    )
    co2_temp_button.pack(side='left', padx=10)
    ...
    acet_alc_co_nh4_toulen_button = tk.Button(
        button_frame,
        text='Wykres Aceton, Alcohol, CO, NH4, Toulen vs Czas',
        command=lambda: switch_plot('acet_alc_co_nh4_toulen'),
    )
    acet_alc_co_nh4_toulen_button.pack(side='left', padx=10)
